...
def plot_distribution(io, expression, xbins, data, num_rolls):
    ...
    ax.hist(centroids, bins=len(data), weights=data, range=(xbins[0], xbins[-1]), color="#d80404", zorder=1, density=True)
    ax.hist(centroids, bins=len(data), weights=data, range=(xbins[0], xbins[-1]), color="#7f0707", zorder=2, histtype="step", density=True)
